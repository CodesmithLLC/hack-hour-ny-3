...
// For example: countTwos(3) should be 1 because there are no 2s in the number one. There is 
// One 2 in the number two and no 2s in the number three.
...
function countTwos(num) {
    ...
    for (let i = 0; i <= num; i += 1){
        if (i.toString().indexOf('2') > -1){
            i.toString().split('').forEach((number) => {
                console.log(number)
